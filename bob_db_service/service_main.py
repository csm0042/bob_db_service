...
class MainTask(object):
    def __init__(self, log_path, **kwargs):
        ...
        if kwargs is not None:
            for key, value in kwargs.items():
                ...
                if key == "db":
                    self.database = value
                    self.log_init.debug('Database set during __init__ '
                                        'to: %s', self.database)
                if key == "msg_in_queue":
    ...
    def run(self):
        ...
        self.log_run.info('Starting database service main task')
        ...
        while True:
            ...
            if self.msg_in_queue.qsize() > 0:
                ...
                if self.msg_type == self.message_types['log_status_update']:
                    self.log_run.debug('Message is a log status update')
                ...
                if self.msg_type == self.message_types['return_command']:
                    self.log_run.debug('Message is a return command')
                ...
                if self.msg_type == self.message_types['update_command']:
                    self.log_run.debug('Message is a update command')
            ...
            if datetime.datetime.now() >= (self.last_check_db + datetime.timedelta(seconds=1)):
                ...
                self.log_run.debug('Performing periodic check of pending commands')
                self.out_msg_list = yield from process_return_command_msg(
                    self.log_path,
                    self.ref_num,
                    self.database,
