...
class MainTask(object):
    def __init__(self, logger=None, **kwargs):
        ...
        if kwargs is not None:
            for key, value in kwargs.items():
                ...
                if key == "db":
                    self.database = value
                    self.logger.debug('Database set during __init__ '
                                      'to: %s', self.database)
                if key == "msg_in_queue":
    ...
    def run(self):
        ...
        self.logger.info('Starting database service main task')
        ...
        while True:
            ...
            if self.msg_in_queue.qsize() > 0:
                ...
                if self.msg_type == self.message_types['log_status_update']:
                    self.logger.debug('Message is a log status update')
                ...
                if self.msg_type == self.message_types['return_command']:
                    self.logger.debug('Message is a return command')
                ...
                if self.msg_type == self.message_types['update_command']:
                    self.logger.debug('Message is a update command')
            ...
            if datetime.datetime.now() \
                >= (self.last_check_hb + datetime.timedelta(seconds=60)):
                self.destinations = [
            ...
            if datetime.datetime.now() >= (self.last_check_db + datetime.timedelta(seconds=1)):
                ...
                self.logger.debug('Performing periodic check of pending commands')
                self.out_msg_list = yield from process_return_command_msg(
                    self.logger,
                    self.ref_num,
                    self.database,
